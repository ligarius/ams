...
import { addDataRequestAttachment } from '@backend/services/dataRequestService';
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const created = await addDataRequestAttachment(projectId, dataRequestId, payload, result.actor);
    ...
    console.error('Failed to add data request attachment', error);
    return NextResponse.json({ message: 'Error inesperado al adjuntar archivo' }, { status: 500 });
  }
}
